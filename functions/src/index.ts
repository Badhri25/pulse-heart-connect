...
export const onPulseWrite = functions.database.ref('/pairs/{pairId}/pulse')
  .onWrite(async (
    ...
    try {
      ...
      if (failures.length) functions.logger.warn('FCM send failures', failures.map((f) => f.error?.message));
    } catch (e:any) {
